...
const GUARDED: Rule[] = [
  // ---- Machine learning. The target audience, so an overclaim here is the expensive one. ----
  {
    ...
    answer:
      "No framework work is documented for him: no PyTorch, TensorFlow, JAX, Keras, or Hugging Face code, and I am not going to imply otherwise. What he has built sits a layer above the framework - deploying and running local models and engineering systems around them. That means Ollama, LM Studio, and Open WebUI on hardware he owns, offline since 2023; the language-to-geometry pipeline where a local model turns a request into structured commands and deterministic C#/.NET code owns the geometry and its validation; Tesseract OCR over messy engineering PDFs with Levenshtein matching on the noisy output; whisper.cpp speech-to-text; and Gemini API pipelines in Digital Twin Pro that turn a photo into structured item and quantity data. If a specific framework is a hard requirement, treat it as something he would be picking up rather than something he has shipped.",
  },
  {
    id: "ml-training",
  ...
  },
  {
    ...
    answer:
      "What he found hard is not recorded, and I would rather not invent a struggle on his behalf. What is documented is what the work actually is: the Digital Twin Pro renderer is hand-written rather than a wrapper around a game engine - SceneRenderer and Projection3D with a Z-sorted painter's-algorithm draw queue over a SQLite inventory model, with the 3D projection and depth sorting implemented by him. On the language-to-geometry side the deliberate choice was keeping the model out of anything that has to be correct, so deterministic code owns construction and validation. Which part fought back hardest is a good question and his to answer at " +
      `${EMAIL}.`,
  },
  {
    id: "handles-feedback",
